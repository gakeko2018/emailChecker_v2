...
let Promise = require('bluebird'),
    ...
    getAddressFromTextFile = require('./readfromfile.js').getAddressFromTextFile("emails.txt"),
    loggerOptions = require('./logger.js').loggerOptions,
    logger = require('./logger.js').logger

if (argv.length === 0) {
  if (getAddressFromTextFile.length === 0)
    throw new Error('You must provide one or more email addresses.')
    else argv = getAddressFromTextFile.map(item=>item.trim());
}
